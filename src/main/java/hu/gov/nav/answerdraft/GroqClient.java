...
final class GroqClient {
    ...
    private String request(Prompt prompt,boolean browserSearch,double temperature){
        Map<String,Object> request=new LinkedHashMap<>();request.put("model",config.groqModel());request.put("messages",List.of(Map.of("role","system","content",prompt.system()),Map.of("role","user","content",prompt.user())));request.put("temperature",temperature);request.put("max_completion_tokens",1200);request.put("stream",false);request.put("reasoning_effort","low");
